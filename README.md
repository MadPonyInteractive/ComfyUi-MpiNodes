...
| **MpiLatentUpscale** | Upscale a latent to a target given in **pixels**, dividing by the model's own VAE `stride` instead of core's hardcoded `// 8`. Two fixes in one node. Core's Upscale Latent crashes on a packed audio+video latent (`AttributeError: 'NestedTensor' object has no attribute 'reshape'`, because `common_upscale` reshapes) — this unbinds the pair, scales only the half with spatial dims (picked by `dim() >= 5`, not by index) and passes the audio half `[B,32,2,T]` through untouched. And core assumes the SD VAE factor, so an H3 target typed in pixels comes out **4x too big** with no error at all — `stride` is the pixels-per-latent-cell END TO END, the VAE's spatial compression TIMES the transformer's patch size: **32 for MiniMax H3** (VAE 16 x DiT patch 2, which is why its canvas rule is a multiple of 32), 8 for the SD/SDXL family. Get it wrong and nothing errors - you silently request the wrong resolution, and an over-upscale decodes as hard horizontal banding, not as softness. Built for the hi-res fix: small stage 1, upscale here, let stage 2's low sigmas finish at the target so only half the denoise pays for the resolution. |
