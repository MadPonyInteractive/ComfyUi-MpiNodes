...
- MpiLatentUpscale - stride now defaults to 32, not 16. It is pixels per latent cell END TO END - the VAE's spatial compression TIMES the transformer's patch size - and MiniMax H3 is VAE 16 x DiT patch (1,2,2) = 32, which is exactly why its canvas rule is a multiple of 32. 16 read the VAE factor alone and silently requested a 2x over-upscale, which decodes as hard horizontal banding rather than as softness, so it reads as an interpolation artifact and survives every attempt to fix it by changing upscale_method
